...
const StatCard = ({ title, value, icon, color, textColor }) => {
  ...
  return (
    <div className={`${color} h-full rounded-lg shadow-md border-l-4 border-l-${textColor.replace('text-', 'border-')} border-t-0 border-r-0 border-b-0 transition-all hover:shadow-lg relative`}>
      ...
      {/* Tooltip */}
      {showTooltip && (
        <div className="absolute z-50 bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-sm rounded-lg shadow-lg max-w-xs whitespace-normal">
